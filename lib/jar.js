...
(function() {
  var Jar, ScriptNode, StyleNode, crypto, death, fs, nib, path, stylus, uglify;
  ...
  module.exports = Jar = (function() {
    ...
    */
    ...
    Jar.prototype.build = function() {
      var endTime, startTime, v, _i, _len, _ref;
      ...
      this.addStyleDependency(this.options.style);
      if (this.production) {

      } else {
        this.buildDevelopment(this.jsTagList, this.vendorDependencies, 'vendor');
        this.buildDevelopment(this.jsTagList, this.userDependencies, 'user');
        this.buildDevelopment(this.cssTagList, this.styleDependencies, 'style');
      }
      this.buildInitialize();
      this.jsTags = this.jsTagList.join('');
      this.cssTags = this.cssTagList.join('');
      endTime = new Date().getTime();
      return sugar.info(("Blender rebuild [" + this.name + "]: ").blue, ("" + (endTime - startTime) + "ms").grey);
    };
    ...
    */
    ...
    Jar.prototype.addStyleDependency = function(pathName) {
      var node;
      sugar.info("add style dep: " + pathName);
      node = new StyleNode(pathName, this.options.dir);
      this.styleDependencies[pathName] = node;
      this.watch(node);
      return node;
    };
    ...
    */
    ...
    Jar.prototype.buildProduction = function(dependencies, description, minify) {
      var buffer, fileName, filePath, hash, k, node, src;
      if (minify == null) {
        minify = false;
      }
      buffer = '';
      ...
      filePath = path.join(this.buildPath, fileName);
      fs.writeFile(path.resolve(filePath), buffer, function(err) {
        if (err) {
          throw err;
        }
        return sugar.info(("Blender write production: " + description).blue);
      });
      src = "" + this.buildHost + "/" + fileName;
